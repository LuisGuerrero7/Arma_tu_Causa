...
// export default OrderCausa;
import React, { useContext } from "react";
import { Context } from "../store/appContext";
import toast from 'react-hot-toast';

const IngredientRadius = ({ name, price, papaValue, onUnChecked, papaId }) => {
    return (
        <div className="col-12 my-2">
            <div className="row border-bottom border-dark">
                <div className="col-6 m-0 p-0">
                ...
                <div className="form-check col-6 justify-content-end d-flex">
                    <input className="form-check-input form-check-input-checked-bg-success"
                        type="checkbox" checked={papaValue === papaId} id={"papa-" + name}
                        onChange={() => {
                            onUnChecked();
                        }}
                    />
...
const IngredientOption = ({ name, price, onSelect }) => {
    return (
        <div className="col-12 my-2">
            <div className="row border-bottom border-dark">
                <div className="col-6 m-0 p-0">
    ...
    return (
        <div className="container mx-auto">
            <div className="row mx-auto col-6">
                ...
                <div className="col-12 mt-4 bg-dark text-white d-flex flex-column justify-content-center align-items-start p-2">
                    <h1 className="fw-normal fs-5 m-0 p-0">Papa</h1>
                </div>
                {
                    store.papa.map((papa, index) =>
                        <IngredientRadius key={index} {...papa}
                            papaValue={order.papa}
                            papaId={papa.id}
                            onUnChecked={() => {
                                setOrder({
                                    ...order,
                ...
                <div className="col-12 mt-4 bg-dark text-white d-flex flex-column justify-content-center align-items-start p-2">
                    <h1 className="fw-normal fs-5 m-0 p-0">Proteínas</h1>
                </div>
                {
                    store.proteina.map((protein, index) =>
                        <IngredientOption key={index} {...protein}
                            onSelect={(checked) => onSelectedKey(checked, 'proteins', protein.id)}
                        />
                    )
                ...
                {
                    store.verduras.map((vegetable, index) =>
                        <IngredientOption key={index} {...vegetable}
                            onSelect={(checked) => onSelectedKey(checked, 'veggies', vegetable.id)}
                        />
                    )
                }
                ...
                {
                    store.salsas.map((salsa, index) =>
                        <IngredientOption key={index} {...salsa}
                            onSelect={(checked) => onSelectedKey(checked, 'salsas', salsa.id)}
                        />
                    )
                }

                <div className="col-12 mt-4 bg-dark text-white d-flex flex-column justify-content-center align-items-start p-2">
                    <h1 className="fw-normal fs-5 m-0 p-0">Toppings</h1>
                </div>
                {
                    store.toppings.map((topping, index) =>
                        <IngredientOption key={index} {...topping}
                            onSelect={(checked) => onSelectedKey(checked, 'toppings', topping.id)}
                        />
                    )
                }
